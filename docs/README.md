# role-assignment-service/Docs

Documentation for the role-assignment-service service
## BDD
...
When the image is run you should be able to reach the dockerized _role-assignment-service_ exposed on `http://localhost:3008`.

If you already added the `127.0.0.1 role-assignment-service` entry in your `/etc/hosts` then the _role-assignment-service_ is reachable on `http://role-assignment-service:3008`.
