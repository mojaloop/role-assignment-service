...
describe('shared/logger', (): void => {
  ...
  it('should log response via JSON.stringify', (): void => {
    ...
    expect(logger.info).toBeCalledWith(
      `RAS-Trace - Response: ${JSON.stringify(request.response.source)} Status: ${request.response.statusCode}`
    )
  })
  ...
  it('should log response via inspect', (): void => {
    ...
    expect(logger.info).toBeCalledWith(
      `RAS-Trace - Response: ${inspect(request.response.source)} Status: ${request.response.statusCode}`
    )
  })
  ...
  it('should log if there is no request.response', (): void => {
    ...
    logResponse(request as RequestLogged)
    expect(spyStringify).toBeCalled()
    expect(logger.info).toBeCalledWith(`RAS-Trace - Response: ${request.response.toString()}`)
  })
